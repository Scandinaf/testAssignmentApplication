...
object Main extends App with LazyLogging {
  (for {
    ...
    ancestor = "com.eg.assignment.ShortestPath"
  ...
  private def findImplementation(ancestor: String): Either[Exception, ShortestPath] =
    ImplementationPreProcessor(ancestor).getProcessResult

  private def runTests(instance: ShortestPath): Either[Exception, List[TestSuiteResult]] =
