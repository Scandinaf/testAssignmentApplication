...
class ImplementationPreProcessor(ancestor: String) extends PreProcessor[Either[Exception, ShortestPath]] with LazyLogging {
  override val getProcessResult: Either[Exception, ShortestPath] = {
    tryAllExceptions(ClassFinderHelper.findSubclasses(ancestor) match {
      ...
      case classInfo :: Nil => ClassInstanceHelper.newInstance[ShortestPath](classInfo.name)
...
object ImplementationPreProcessor {
  def apply(ancestor: String): ImplementationPreProcessor =
    new ImplementationPreProcessor(ancestor)
}
