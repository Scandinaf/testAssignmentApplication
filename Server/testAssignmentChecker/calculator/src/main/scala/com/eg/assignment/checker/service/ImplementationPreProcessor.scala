...
class ImplementationPreProcessor(ancestor: String) extends PreProcessor[Either[Exception, Calculator]] with LazyLogging {
  override val getProcessResult: Either[Exception, Calculator] = {
    tryAllExceptions(ClassFinderHelper.findSubclasses(ancestor) match {
      ...
      case classInfo :: Nil => ClassInstanceHelper.newInstance[Calculator](classInfo.name)
